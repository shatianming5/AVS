...
def main(argv: list[str] | None = None) -> int:
    ...
    arr = np.asarray([1.0 if r["correct"] else 0.0 for r in rows], dtype=np.float64)
    arr_del = np.asarray([1.0 if r["correct_deleted"] else 0.0 for r in rows], dtype=np.float64)
    invalid = np.asarray([1.0 if r["invalid"] else 0.0 for r in rows], dtype=np.float64)
    invalid_del = np.asarray([1.0 if r["invalid_deleted"] else 0.0 for r in rows], dtype=np.float64)
    changed = np.asarray([1.0 if r["pred_changed"] else 0.0 for r in rows], dtype=np.float64)

    acc = float(arr.mean() if arr.size else 0.0)
    acc_del = float(arr_del.mean() if arr_del.size else 0.0)
    payload = {
        "ok": True,
        ...
        "invalid_rate_deleted": float(invalid_del.mean() if invalid_del.size else 0.0),
        "pred_change_rate": float(changed.mean() if changed.size else 0.0),
        "artifacts": {"rows_jsonl": str(out_dir / "rows.jsonl"), "faithfulness_json": str(out_dir / "faithfulness.json")},
    }
