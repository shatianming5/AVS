    ...
    if method == "ast_lr":
        ...
        for i, cid in enumerate(clip_ids):
            ...
            if (i + 1) % 200 == 0 or (i + 1) == len(clip_ids):
                dt = _time.time() - t0
                print(f"[scores] {i+1}/{len(clip_ids)} clips ({dt:.1f}s)", flush=True)
    elif method == "av_wavlm_clip_lossgain_mlp":
        # Oracle-distilled Stage-1: predict per-second *visual usefulness* as loss-gain from swapping in higher-res
        # vision features, but using only cheap deployable features (WavLM audio embeddings + low-res CLIP embeddings).
        #
        # This is qualitatively different from eventness (label != 0): it tries to predict when spending more visual
        # tokens actually reduces classification loss, which is closer to the downstream objective behind C0003.
        if caches_dir is None:
            raise ValueError(f"{method} requires caches_dir to load CLIP features")
        if train_ids is None or labels_by_clip is None:
            raise ValueError(f"{method} requires train_ids and labels_by_clip")
        ...
        from avs.experiments.ave_p0 import _train_audio_basic_mlp_visgain_eventness
        ...
        for i, cid in enumerate(train_ids):
            ...
            if (i + 1) % 200 == 0 or (i + 1) == len(train_ids):
                dt = _time.time() - t0
                print(f"[{method}] feats+teacher train {i+1}/{len(train_ids)} clips ({dt:.1f}s)", flush=True)

        # Teacher: base-res classifier loss reduction when swapping in high-res features (event seconds only).
        ...
        _ = train_per_segment_classifier(
            ...
            x_val=x_base_t,
            y_val=y_t,
            cfg=TrainConfig(epochs=20, batch_size=256, lr=2e-3, weight_decay=0.0),
        )
        ...
        teacher_by_train = {
            str(cid): gain[int(i)].detach().cpu().numpy().astype(np.float32, copy=False) for i, cid in enumerate(train_ids)
        }
        ...
        for i, cid in enumerate(clip_ids):
            ...
            if (i + 1) % 200 == 0 or (i + 1) == len(clip_ids):
                dt = _time.time() - t0
                print(f"[scores] {i+1}/{len(clip_ids)} clips ({dt:.1f}s)", flush=True)
    elif method == "av_wavlm_clip_mil_mlp":
        # Supervised A+V eventness with a MIL objective (clip-wise peakiness) on strong frozen backbones:
        # WavLM audio embeddings + low-res CLIP embeddings.
        #
        # Compared to BCE-style per-second supervision, MIL is more aligned with Top-K anchor selection and
        # can yield peaky score distributions (less fallback dilution), which is necessary for pushing C0003.
        if caches_dir is None:
            raise ValueError(f"{method} requires caches_dir to load CLIP features")
        if train_ids is None or labels_by_clip is None:
            raise ValueError(f"{method} requires train_ids and labels_by_clip")
        ...
        from avs.audio.wavlm_probe import WavLMEmbeddingProbe, WavLMProbeConfig
        from avs.experiments.ave_p0 import _train_audio_basic_mil_mlp_eventness
        ...
        for i, cid in enumerate(clip_ids):
            ...
            if (i + 1) % 200 == 0 or (i + 1) == len(clip_ids):
                dt = _time.time() - t0
                print(f"[scores] {i+1}/{len(clip_ids)} clips ({dt:.1f}s)", flush=True)
    elif method == "av_wavlm_clip_xattn_mil":
        # Explicit cross-time A↔V alignment as Stage-1 scoring (MIL objective):
        #
        #   - audio backbone: WavLM embeddings per second
        #   - vision backbone: low-res CLIP embeddings per second
        #   - model: audio-conditioned attention over all visual timestamps (10×10),
        #            then per-second logits trained with a MIL loss (peaky clip-wise separation).
        #
        # This is a qualitative shift from per-second independent heads: the attention can learn soft temporal
        # alignment (audio leads/lags vision) and provide a sharper anchor distribution.
        if caches_dir is None:
            raise ValueError(f"{method} requires caches_dir to load CLIP features")
        if train_ids is None or labels_by_clip is None:
            raise ValueError(f"{method} requires train_ids and labels_by_clip")
        ...
        # Keep defaults backward-compatible with the initial E0902 run; override via env vars.
        ...
        print(
            f"[{method}] vis_res={vis_res} vis_mode={vis_mode} train_device={train_device} epochs={epochs} bs={bs}",
            flush=True,
        )
        ...
        def _load_vis_feats_npz_low(cache_path: Path) -> np.ndarray:
            with np.load(cache_path) as z:
                ...
                if key in z.files:
                    ...
                    # Keep a safe fallback for older caches while making the chosen resolution explicit.
            ...
            # CLIPdiff: per-second motion proxy from consecutive differences.
            # Keep it scale-stable with per-step l2-norm after differencing.
        ...
        # Load (possibly augmented) CLIP features for train + eval ids once (I/O dominates; keep it deterministic).
        vis_ids = sorted(set(train_ids + [str(x) for x in clip_ids]))
        vis_low_by_clip: dict[str, np.ndarray] = {}
        for i, cid in enumerate(vis_ids):
            cache_path = caches_dir / f"{cid}.npz"
            if not cache_path.exists():
                raise FileNotFoundError(f"missing cache: {cache_path}")
            v_low = _load_vis_feats_npz_low(cache_path)
            ...
            vis_low_by_clip[cid] = _l2norm(v_low)
            if (i + 1) % 500 == 0 or (i + 1) == len(vis_ids):
                dt = _time.time() - t0
                print(f"[{method}] vis_low {i+1}/{len(vis_ids)} clips ({dt:.1f}s)", flush=True)

        # Training set labels (binary per second).
        y_by_train: dict[str, np.ndarray] = {}
        for cid in train_ids:
            labs = np.asarray(labels_by_clip[cid], dtype=np.int64)[: int(num_segments)]
            y_by_train[cid] = (labs != 0).astype(np.float32, copy=False)

        # Model: audio-conditioned cross-time attention over vision.
        a0 = wavlm_emb_by_clip.get(train_ids[0])
        if a0 is None:
            raise ValueError("missing wavlm embeddings for training")
        v0 = vis_low_by_clip.get(train_ids[0])
        if v0 is None:
            raise ValueError("missing clip embeddings for training")
        audio_dim = int(a0.shape[-1])
        vision_dim = int(v0.shape[-1])

        class _AVXAttnMIL(nn.Module):
            def __init__(self, *, proj_dim: int = 128, dropout: float = 0.1) -> None:
                super().__init__()
                ...
                self.head = nn.Sequential(
                    nn.Linear(int(3 * proj_dim), int(proj_dim)),
                    nn.ReLU(),
        ...
        model = _AVXAttnMIL(proj_dim=128, dropout=0.1).to(device)
        opt = torch.optim.AdamW(model.parameters(), lr=2e-3, weight_decay=0.0)

        # MIL loss per batch:
        #   loss = -log sum_{t in P} softmax(scores)[t]
        ...
        for _epoch in range(int(epochs)):
            ...
            for step in range(int(steps)):
                idx = perm[step * bs : (step + 1) * bs].tolist()
                ...
                y_np = np.stack([y_by_train[c] for c in cids], axis=0).astype(np.float32, copy=False)  # [B,T]
                ...
                logits = model(a_t, v_t)  # [B,T]
                scores_all = torch.logsumexp(logits, dim=1)  # [B]
                pos = y_t > 0.5
                scores_pos = logits.masked_fill(~pos, float("-inf"))
                scores_pos_lse = torch.logsumexp(scores_pos, dim=1)  # [B]
        ...
        for i0 in range(0, len(clip_ids), int(eval_bs)):
            ...
            v_np = np.stack([vis_low_by_clip[c] for c in batch], axis=0).astype(np.float32, copy=False)
            with torch.no_grad():
                logits = (
                    model_cpu(torch.from_numpy(a_np).float(), torch.from_numpy(v_np).float())
                    .detach()
                    .cpu()
                    .numpy()
                    .astype(np.float32)
            ...
            for j, cid in enumerate(batch):
                out[cid] = [float(x) for x in logits[int(j)].tolist()]
        ...
        for i0 in range(0, len(clip_ids), int(eval_bs)):
            ...
            if (i0 + len(batch)) % 500 == 0 or (i0 + len(batch)) == len(clip_ids):
                dt = _time.time() - t0
                print(f"[scores] {i0+len(batch)}/{len(clip_ids)} clips ({dt:.1f}s)", flush=True)
    elif method == "av_wavlm_clip_xattn_evt":
        # Explicit cross-time A↔V alignment as Stage-1 scoring (supervised binary eventness):
        #
        #   - audio backbone: WavLM embeddings per second
        #   - vision backbone: low-res CLIP embeddings per second (optionally + CLIPdiff)
        #   - model: audio-conditioned attention over all visual timestamps (10×10),
        #            then per-second eventness logits trained with a BCE loss.
        #
        # Additional MIL-style clip loss encourages a peakier score distribution:
        #   - clip_logit = max_t(logit[t]); clip_label = any_t(label[t] != 0)
        #
        # Score is the per-second eventness logit s(t), intended to align with oracle_top2 (label!=0).
        if caches_dir is None:
            raise ValueError(f"{method} requires caches_dir to load CLIP features")
        if train_ids is None or labels_by_clip is None:
            raise ValueError(f"{method} requires train_ids and labels_by_clip")
        ...
        proj_dim = int(os.environ.get("XATTN_PROJ_DIM", "128"))
        dropout = float(os.environ.get("XATTN_DROPOUT", "0.1"))
        clip_loss_weight = float(os.environ.get("XATTN_EVT_CLIP_LOSS_WEIGHT", "0.5"))
        print(
            f"[{method}] vis_res={vis_res} vis_mode={vis_mode} train_device={train_device} "
            f"epochs={epochs} bs={bs} eval_bs={eval_bs} lr={lr} proj_dim={proj_dim} dropout={dropout} "
            f"clip_loss_w={clip_loss_weight}",
            flush=True,
        )
        ...
        for i, cid in enumerate(train_ids):
            ...
            if labs is None:
                raise ValueError(f"{method}: missing labels for train clip_id={cid!r}")
            y = np.asarray([(int(x) != 0) for x in labs], dtype=np.float32)[: int(num_segments)]
            if int(y.shape[0]) != int(num_segments):
                raise ValueError(f"{method}: bad label shape for {cid}: {y.shape} expected ({int(num_segments)},)")
            y_by_train[cid] = y
            ...
            seg_pos += pos
            seg_neg += float(int(num_segments)) - pos
            if float(y.max().item()) > 0.5:
                clip_pos += 1.0
            else:
                clip_neg += 1.0
        ...
        pos_weight = torch.tensor([float(seg_neg) / max(1.0, float(seg_pos))], dtype=torch.float32)
        clip_pos_weight = torch.tensor([float(clip_neg) / max(1.0, float(clip_pos))], dtype=torch.float32)

        class _AVXAttnEvt(nn.Module):
            def __init__(self, *, proj_dim: int, dropout: float):
                super().__init__()
                self.proj_dim = int(proj_dim)
                self.audio_proj = nn.Sequential(
                    nn.Linear(int(wavlm_emb_by_clip[train_ids[0]].shape[-1]), int(proj_dim)),
                ...
                self.drop = nn.Dropout(p=float(dropout))
                self.head = nn.Linear(int(3 * proj_dim), 1)
        ...
        torch.manual_seed(0)
        device = torch.device(str(train_device))
        model = _AVXAttnEvt(proj_dim=int(proj_dim), dropout=float(dropout)).to(device)
        opt = torch.optim.AdamW(model.parameters(), lr=float(lr), weight_decay=0.0)
        seg_loss_fn = nn.BCEWithLogitsLoss(pos_weight=pos_weight.to(device=device, dtype=torch.float32))
        clip_loss_fn = nn.BCEWithLogitsLoss(pos_weight=clip_pos_weight.to(device=device, dtype=torch.float32))
        ...
        for _epoch in range(int(epochs)):
            ...
            for step in range(int(steps)):
                ...
                a_np = np.stack([_l2norm(wavlm_emb_by_clip[c]) for c in cids], axis=0).astype(np.float32, copy=False)
                v_np = np.stack([vis_low_by_clip[c] for c in cids], axis=0).astype(np.float32, copy=False)
                y_np = np.stack([y_by_train[c] for c in cids], axis=0).astype(np.float32, copy=False)
                ...
                logits = model(a_t, v_t)  # [B,T]
                seg_loss = seg_loss_fn(logits.reshape(-1), y_t.reshape(-1))
                loss = seg_loss
                if float(clip_loss_weight) > 0.0:
                    clip_logits = logits.max(dim=1).values  # [B]
                    clip_targets = y_t.max(dim=1).values  # [B]
                    clip_loss = clip_loss_fn(clip_logits, clip_targets)
        ...
        for i0 in range(0, len(clip_ids), int(eval_bs)):
            ...
            with torch.no_grad():
                logits = model_cpu(torch.from_numpy(a_np).float(), torch.from_numpy(v_np).float())  # [B,T]
                logits_np = logits.detach().cpu().numpy().astype(np.float32)

            for j, cid in enumerate(batch):
                out[cid] = [float(x) for x in logits_np[int(j)].tolist()]
        ...
        for i, cid in enumerate(clip_ids):
            ...
            if (i + 1) % 200 == 0 or (i + 1) == len(clip_ids):
                dt = _time.time() - t0
                print(f"[scores] {i+1}/{len(clip_ids)} clips ({dt:.1f}s)", flush=True)
    elif method == "av_wavlm_clip_avel_bilstm_cls_target":
        # AVE-localizer style Stage-1: audio+vision fusion + temporal modeling.
        #
        # Inspired by AVE-ECCV18 (A+V-att): fuse audio+visual evidence and use a sequence model to predict
        # per-second event class; convert logits to a scalar score via class-conditional margin.
        #
        # This is intentionally different from per-second MLP/TCN heads: the BiLSTM can leverage local temporal
        # context when the raw per-second signal is weak/noisy, which is exactly the failure mode behind C0003.
        if caches_dir is None:
            raise ValueError(f"{method} requires caches_dir to load CLIP features")
        if train_ids is None or labels_by_clip is None:
            raise ValueError(f"{method} requires train_ids and labels_by_clip")
        ...
        vis_res = int(os.environ.get("AVEL_VIS_RES", "160"))
        vis_mode = str(os.environ.get("AVEL_VIS_FEATS", "clip")).strip().lower()
        if vis_mode not in ("clip", "clipdiff", "clip+clipdiff"):
            raise ValueError(f"{method}: unknown AVEL_VIS_FEATS={vis_mode!r}; expected clip|clipdiff|clip+clipdiff")
        train_device = str(os.environ.get("AVEL_TRAIN_DEVICE", "cpu"))
        epochs = int(os.environ.get("AVEL_EPOCHS", "40"))
        bs = int(os.environ.get("AVEL_BS", "256"))
        eval_bs = int(os.environ.get("AVEL_EVAL_BS", "256"))
        lr = float(os.environ.get("AVEL_LR", "1e-3"))
        proj_dim = int(os.environ.get("AVEL_PROJ_DIM", "128"))
        hidden_dim = int(os.environ.get("AVEL_HIDDEN_DIM", "128"))
        dropout = float(os.environ.get("AVEL_DROPOUT", "0.1"))

        print(
            f"[{method}] vis_res={vis_res} vis_mode={vis_mode} train_device={train_device} "
            f"epochs={epochs} bs={bs} lr={lr} proj_dim={proj_dim} hidden_dim={hidden_dim} dropout={dropout}",
            flush=True,
        )
        ...
        vis_ids = sorted(set(train_ids + [str(x) for x in clip_ids]))
        vis_by_clip: dict[str, np.ndarray] = {}
        for i, cid in enumerate(vis_ids):
            cache_path = caches_dir / f"{cid}.npz"
            if not cache_path.exists():
                raise FileNotFoundError(f"missing cache: {cache_path}")
            ...
            vis_by_clip[cid] = _l2norm(v)
            if (i + 1) % 500 == 0 or (i + 1) == len(vis_ids):
                dt = _time.time() - t0
                print(f"[{method}] vis {i+1}/{len(vis_ids)} clips ({dt:.1f}s)", flush=True)

        # Determine the number of AVE classes from the training labels.
        num_classes = 1
        for cid in train_ids:
            labs = labels_by_clip.get(cid) or []
            if labs:
        ...
        if int(num_classes) <= 1:
            raise ValueError(f"{method}: num_classes must be > 1, got {num_classes}")

        y_by_train: dict[str, np.ndarray] = {}
        for cid in train_ids:
            labs = np.asarray(labels_by_clip[cid], dtype=np.int64)[: int(num_segments)]
            if labs.shape[0] != int(num_segments):
                raise ValueError(f"{method}: unexpected label shape for {cid}: {labs.shape}")
            y_by_train[cid] = labs

        # Class reweighting (background dominates in AVE).
        y_all = np.concatenate([y_by_train[c] for c in train_ids], axis=0)
        counts = np.bincount(y_all.astype(np.int64), minlength=int(num_classes)).astype(np.float32)
        total = float(counts.sum())
        weights = np.zeros((int(num_classes),), dtype=np.float32)
        nz = counts > 0
        ...
        a0 = wavlm_emb_by_clip.get(train_ids[0])
        v0 = vis_by_clip.get(train_ids[0])
        if a0 is None or v0 is None:
            raise ValueError(f"{method}: missing features for training")
        audio_dim = int(a0.shape[-1])
        vision_dim = int(v0.shape[-1])

        class _AVELBiLSTM(nn.Module):
            def __init__(self) -> None:
                ...
                self.head = nn.Linear(int(2 * hidden_dim), int(num_classes))

            def forward(self, a: torch.Tensor, v: torch.Tensor) -> torch.Tensor:
                # a: [B,T,A], v: [B,T,V] -> logits: [B,T,C]
                if a.ndim != 3 or v.ndim != 3:
                    raise ValueError(f"expected a/v with shape [B,T,*], got a={tuple(a.shape)}, v={tuple(v.shape)}")
                a_p = self.audio_proj(a)
                v_p = self.vision_proj(v)
                g = self.gate(torch.cat([a_p, v_p, a_p * v_p], dim=-1))
                fused = g * v_p + (1.0 - g) * a_p
                fused = self.drop(fused)
                h, _ = self.lstm(fused)
                h = self.drop(h)
                return self.head(h)
        ...
        torch.manual_seed(0)
        device = torch.device(str(train_device))
        model = _AVELBiLSTM().to(device)
        opt = torch.optim.AdamW(model.parameters(), lr=float(lr), weight_decay=0.0)
        loss_fn = nn.CrossEntropyLoss(weight=torch.from_numpy(weights).to(device=device, dtype=torch.float32))
        ...
        for _epoch in range(int(epochs)):
            ...
            for step in range(int(steps)):
                ...
                a_np = np.stack([_l2norm(wavlm_emb_by_clip[c]) for c in cids], axis=0).astype(np.float32, copy=False)
                v_np = np.stack([vis_by_clip[c] for c in cids], axis=0).astype(np.float32, copy=False)
                y_np = np.stack([y_by_train[c] for c in cids], axis=0).astype(np.int64, copy=False)
        ...
        for i0 in range(0, len(clip_ids), int(eval_bs)):
            batch = clip_ids[i0 : i0 + int(eval_bs)]
            a_np = np.stack([_l2norm(wavlm_emb_by_clip[c]) for c in batch], axis=0).astype(np.float32, copy=False)
            v_np = np.stack([vis_by_clip[c] for c in batch], axis=0).astype(np.float32, copy=False)
            with torch.no_grad():
                logits = model_cpu(torch.from_numpy(a_np).float(), torch.from_numpy(v_np).float())  # [B,T,C]
                logits_np = logits.detach().cpu().numpy().astype(np.float32)

            for j, cid in enumerate(batch):
                logit_rows = logits_np[int(j)]  # [T,C]
                clip_logits = logit_rows.mean(axis=0).astype(np.float32, copy=True)
                clip_logits[0] = float("-inf")
                cls = int(np.argmax(clip_logits).item())
                s = (logit_rows[:, cls] - logit_rows[:, 0]).astype(np.float32, copy=False)
                out[cid] = [float(x) for x in s.tolist()]
