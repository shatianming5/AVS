...
# E0376: Quick test402 reproduction (SEEDS=0..2) for the E0375 winner + diagnosis (E0344).
#
# Usage:
#   bash scripts/e0376_ave_p0_best_to_test_quick_official_vision_binary_mlp.sh
#
...
if [[ -z "${BEST_CONFIG_JSON:-}" ]]; then
  ...
  if [[ -n "${latest_best}" && -f "${latest_best}" ]]; then
    ...
    echo "[e0376] BEST_CONFIG_JSON not set; using ${BEST_CONFIG_JSON}"
  else
...
SEEDS="${SEEDS:-0,1,2}"
OUT_DIR="${OUT_DIR:-runs/E0376_quick_test402_${EVENTNESS}_$(date +%Y%m%d-%H%M%S)}"
