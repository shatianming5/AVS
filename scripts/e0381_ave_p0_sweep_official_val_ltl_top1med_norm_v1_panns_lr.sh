...
# E0381: Stage-1 sweep on official AVE val402 for supervised calibration on pretrained PANNs outputs:
# `EVENTNESS=panns_lr` under the scale-invariant top1-med gate (`candidate_set=ltl_top1med_norm_v1`).
#
# This is a thin wrapper around `scripts/e0207_ave_p0_sweep_official_val_ltl_stage1.sh` that:
#   - defaults EVENTNESS to panns_lr
#   - defaults CANDIDATE_SET to ltl_top1med_norm_v1
#   - defaults AUDIO_DEVICE/TRAIN_DEVICE to cuda:0 when available
#   - writes outputs under runs/E0381_*
...
OUT_DIR="${OUT_DIR:-runs/E0381_ave_p0_sweep_official_val_${EVENTNESS}_${CANDIDATE_SET}_$(date +%Y%m%d-%H%M%S)}"
