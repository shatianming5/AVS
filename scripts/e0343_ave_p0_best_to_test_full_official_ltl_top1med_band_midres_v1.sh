...
#
# Usage:
#   EVENTNESS=av_clipdiff_mlp bash scripts/e0343_ave_p0_best_to_test_full_official_ltl_top1med_band_midres_v1.sh
#
...
if [[ -z "${BEST_CONFIG_JSON:-}" ]]; then
  ...
  if [[ -n "${latest_best}" && -f "${latest_best}" ]]; then
    ...
    echo "[e0343] BEST_CONFIG_JSON not set; using ${BEST_CONFIG_JSON}"
  else
...
OUT_DIR="${OUT_DIR:-runs/E0343_full_test402_${EVENTNESS}_$(date +%Y%m%d-%H%M%S)}"
