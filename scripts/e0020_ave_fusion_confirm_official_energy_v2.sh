...
# Confirm fusion adds on top of the energy_v2-selected sampling config on official AVE test402:
# compares `audio_concat_uniform` vs `audio_concat_anchored_top2`.
#
# Usage:
#   BEST_CONFIG_JSON=runs/E0018_.../best_config.json bash scripts/e0020_ave_fusion_confirm_official_energy_v2.sh
#
# Produces:
#   - runs/E0020_*/metrics.json

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

if [[ -z "${BEST_CONFIG_JSON:-}" ]]; then
  latest_best="$(ls -t runs/E0018_*/best_config.json 2>/dev/null | head -n 1 || true)"
  if [[ -n "${latest_best}" && -f "${latest_best}" ]]; then
    BEST_CONFIG_JSON="${latest_best}"
    echo "[e0020] BEST_CONFIG_JSON not set; using ${BEST_CONFIG_JSON}"
  else
    echo "ERROR: BEST_CONFIG_JSON is required (path to best_config.json from E0018)" >&2
    exit 2
  fi
fi
...
OUT_DIR="${OUT_DIR:-runs/E0020_ave_fusion_confirm_official_test_energy_v2_$(date +%Y%m%d-%H%M%S)}"
