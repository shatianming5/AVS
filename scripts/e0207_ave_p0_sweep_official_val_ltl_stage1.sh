...
# E0207: Fixed-space sweep on official AVE val402 for a given Stage-1 eventness method.
#
# Goal: Find a stronger anchored_top2 configuration (Stage-2 plan knobs) under the same downstream protocol
# while varying Stage-1 anchors (e.g., av_clipdiff_lr / av_clipdiff_mlp / av_fused_clipdiff_prod / moe_energy_clipdiff).
#
# Produces:
#   - runs/E0207_*/sweep_summary.json
#   - runs/E0207_*/best_config.json
#   - runs/E0207_*/eventness_scores.json (cached per-second scores; reused across candidates and later test reproduction)
...
LATEST_OFFICIAL_PROCESSED="$(ls -td runs/REAL_AVE_OFFICIAL_RERUN_*/processed 2>/dev/null | head -n 1 || true)"
if [[ -n "${LATEST_OFFICIAL_PROCESSED}" && -d "${LATEST_OFFICIAL_PROCESSED}" ]]; then
  DEFAULT_PROCESSED="${LATEST_OFFICIAL_PROCESSED}"
elif [[ -d "runs/REAL_AVE_OFFICIAL_20260201-124535/processed" ]]; then
...
LATEST_OFFICIAL_BASE="${LATEST_OFFICIAL_PROCESSED%/processed}"
if [[ -n "${LATEST_OFFICIAL_BASE}" && -d "${LATEST_OFFICIAL_BASE}/caches_112_160_224_352_448" ]]; then
  DEFAULT_CACHES="${LATEST_OFFICIAL_BASE}/caches_112_160_224_352_448"
elif [[ -d "runs/REAL_AVE_OFFICIAL_20260201-124535/caches_112_160_224_352_448" ]]; then
...
# Stage-1 anchor proposal method (see avs/experiments/ave_p0.py --eventness-method choices).
...
# Stage-2 candidate set for the sweep (see avs/experiments/ave_p0_sweep.py).
# Default keeps backward compatibility, but learned Stage-1 methods often need `ltl_gini_v1` to avoid excessive fallback.
CANDIDATE_SET="${CANDIDATE_SET:-energy_v3}"
...
OUT_DIR="${OUT_DIR:-runs/E0207_ave_p0_sweep_official_val_${EVENTNESS}_$(date +%Y%m%d-%H%M%S)}"
