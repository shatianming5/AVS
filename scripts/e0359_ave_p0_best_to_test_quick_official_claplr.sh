...
# E0359: Quick test402 reproduction (SEEDS=0..2) for the E0358 winner (clap_lr).
#
# Usage:
#   BEST_CONFIG_JSON=runs/E0358_.../best_config.json bash scripts/e0359_ave_p0_best_to_test_quick_official_claplr.sh
#
# Produces:
#   - runs/E0359_*/metrics.json
...
if [[ -z "${BEST_CONFIG_JSON:-}" ]]; then
  ...
  if [[ -n "${latest_best}" && -f "${latest_best}" ]]; then
    ...
    echo "[e0359] BEST_CONFIG_JSON not set; using ${BEST_CONFIG_JSON}"
  else
...
OUT_DIR="${OUT_DIR:-runs/E0359_quick_test402_${EVENTNESS}_$(date +%Y%m%d-%H%M%S)}"
export OUT_DIR
