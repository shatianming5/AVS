...
# E0351: Full test402 reproduction (SEEDS=0..9) for the E0349 winner (av_clap_clip_agree; k=1 set).
#
# Usage:
#   BEST_CONFIG_JSON=runs/E0349_.../best_config.json bash scripts/e0351_ave_p0_best_to_test_full_official_clapagree_k1.sh
#
# Produces:
#   - runs/E0351_*/metrics.json
...
if [[ -z "${BEST_CONFIG_JSON:-}" ]]; then
  ...
  if [[ -n "${latest_best}" && -f "${latest_best}" ]]; then
    ...
    echo "[e0351] BEST_CONFIG_JSON not set; using ${BEST_CONFIG_JSON}"
  else
...
OUT_DIR="${OUT_DIR:-runs/E0351_full_test402_${EVENTNESS}_k1_$(date +%Y%m%d-%H%M%S)}"
export OUT_DIR
