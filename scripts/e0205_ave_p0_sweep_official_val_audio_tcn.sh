...
# E0205: Fixed-space sweep for audio-TCN Stage-1 on official AVE val402.
#
# Goal: Find a stronger anchored_top2 configuration for a learned audio eventness method
# (audio_basic_tcn / audio_fbank_tcn) under the same downstream protocol as energy_v3 sweeps.
#
# Produces:
#   - runs/E0205_*/sweep_summary.json
#   - runs/E0205_*/best_config.json
#   - runs/E0205_*/eventness_scores.json (cached per-second scores; reused across candidates)
...
# Match the strongest known official full-split protocol.
EPOCHS="${EPOCHS:-5}"
...
# audio_*_tcn runs use CPU audio features by default; override if desired.
...
OUT_DIR="${OUT_DIR:-runs/E0205_ave_p0_sweep_official_val_${EVENTNESS}_$(date +%Y%m%d-%H%M%S)}"

python -m avs.experiments.ave_p0_sweep sweep \
  --candidate-set energy_v3 \
  --meta-dir "${META_DIR}" \
