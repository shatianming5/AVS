...
# E0401: Quick test402 transfer (SEEDS=0..2) for E0400 winner + diagnosis.
...
if [[ -z "${BEST_CONFIG_JSON:-}" ]]; then
  ...
  if [[ -n "${latest_best}" && -f "${latest_best}" ]]; then
    ...
    echo "[e0401] BEST_CONFIG_JSON not set; using ${BEST_CONFIG_JSON}"
  else
...
OUT_DIR="${OUT_DIR:-runs/E0401_quick_test402_${EVENTNESS}_$(date +%Y%m%d-%H%M%S)}"
