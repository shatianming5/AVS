...
#
# Usage:
#   BEST_CONFIG_JSON=runs/E0355_.../best_config.json bash scripts/e0357_ave_p0_best_to_test_full_official_clapevt_k1.sh
#
# Produces:
#   - runs/E0357_*/metrics.json
...
if [[ -z "${BEST_CONFIG_JSON:-}" ]]; then
  ...
  if [[ -n "${latest_best}" && -f "${latest_best}" ]]; then
    ...
    echo "[e0357] BEST_CONFIG_JSON not set; using ${BEST_CONFIG_JSON}"
  else
...
OUT_DIR="${OUT_DIR:-runs/E0357_full_test402_${EVENTNESS}_k1_$(date +%Y%m%d-%H%M%S)}"
export OUT_DIR
