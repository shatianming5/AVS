...
# Reproduce the best config selected by E0018 (energy_v2 sweep) on official AVE test402.
#
# Usage:
#   BEST_CONFIG_JSON=runs/E0018_.../best_config.json bash scripts/e0019_ave_p0_best_to_test_official_energy_v2.sh
#
# Produces:
#   - runs/E0019_*/metrics.json
...
if [[ -z "${BEST_CONFIG_JSON:-}" ]]; then
  ...
  if [[ -n "${latest_best}" && -f "${latest_best}" ]]; then
    ...
    echo "[e0019] BEST_CONFIG_JSON not set; using ${BEST_CONFIG_JSON}"
  else
...
OUT_DIR="${OUT_DIR:-runs/E0019_ave_p0_best_to_test_official_energy_v2_$(date +%Y%m%d-%H%M%S)}"
