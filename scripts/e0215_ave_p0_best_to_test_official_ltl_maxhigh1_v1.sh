...
# E0215: Reproduce the best config selected by E0214 on official AVE test402.
#
# Usage:
#   BEST_CONFIG_JSON=runs/E0214_.../best_config.json EVENTNESS=av_clipdiff_mlp bash scripts/e0215_ave_p0_best_to_test_official_ltl_maxhigh1_v1.sh
#
# Produces:
#   - runs/E0215_*/metrics.json

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "${REPO_ROOT}"

if [[ -z "${BEST_CONFIG_JSON:-}" ]]; then
  latest_best="$(ls -t runs/E0214_*/best_config.json 2>/dev/null | head -n 1 || true)"
  if [[ -n "${latest_best}" && -f "${latest_best}" ]]; then
    BEST_CONFIG_JSON="${latest_best}"
    echo "[e0215] BEST_CONFIG_JSON not set; using ${BEST_CONFIG_JSON}"
  else
    echo "ERROR: BEST_CONFIG_JSON is required (path to best_config.json from E0214)" >&2
    exit 2
  fi
fi
...
OUT_DIR="${OUT_DIR:-runs/E0215_ave_p0_best_to_test_official_${EVENTNESS}_ltl_maxhigh1_v1_$(date +%Y%m%d-%H%M%S)}"
