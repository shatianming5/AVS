...
# Reproduce the best config selected by E0021 (energy_v3 sweep) on official AVE test402.
#
# Usage:
#   BEST_CONFIG_JSON=runs/E0021_.../best_config.json bash scripts/e0022_ave_p0_best_to_test_official_energy_v3.sh
#
# Produces:
#   - runs/E0022_*/metrics.json
...
if [[ -z "${BEST_CONFIG_JSON:-}" ]]; then
  ...
  if [[ -n "${latest_best}" && -f "${latest_best}" ]]; then
    ...
    echo "[e0022] BEST_CONFIG_JSON not set; using ${BEST_CONFIG_JSON}"
  else
...
# Match the sweep default unless overridden.
EPOCHS="${EPOCHS:-5}"
...
OUT_DIR="${OUT_DIR:-runs/E0022_ave_p0_best_to_test_official_energy_v3_$(date +%Y%m%d-%H%M%S)}"
