...
# E0398: Full test402 reproduction (SEEDS=0..9) for the E0396 winner + diagnosis (E0344).
#
...
if [[ -z "${BEST_CONFIG_JSON:-}" ]]; then
  ...
  if [[ -n "${latest_best}" && -f "${latest_best}" ]]; then
    ...
    echo "[e0398] BEST_CONFIG_JSON not set; using ${BEST_CONFIG_JSON}"
  else
...
OUT_DIR="${OUT_DIR:-runs/E0398_full_test402_${EVENTNESS}_k1ext_$(date +%Y%m%d-%H%M%S)}"
