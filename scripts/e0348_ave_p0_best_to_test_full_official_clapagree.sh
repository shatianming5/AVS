...
# E0348: Full test402 reproduction (SEEDS=0..9) for the E0346 winner (av_clap_clip_agree).
#
# Usage:
#   BEST_CONFIG_JSON=runs/E0346_.../best_config.json bash scripts/e0348_ave_p0_best_to_test_full_official_clapagree.sh
#
# Produces:
#   - runs/E0348_*/metrics.json
...
if [[ -z "${BEST_CONFIG_JSON:-}" ]]; then
  ...
  if [[ -n "${latest_best}" && -f "${latest_best}" ]]; then
    ...
    echo "[e0348] BEST_CONFIG_JSON not set; using ${BEST_CONFIG_JSON}"
  else
...
OUT_DIR="${OUT_DIR:-runs/E0348_full_test402_${EVENTNESS}_$(date +%Y%m%d-%H%M%S)}"
export OUT_DIR
