...
# E0382: Quick test402 reproduction (SEEDS=0..2) for the E0381 winner + diagnosis (E0344).
#
# Usage:
#   bash scripts/e0382_ave_p0_best_to_test_quick_official_panns_lr.sh
#   # or with overrides:
#   BEST_CONFIG_JSON=... SEEDS=0,1,2 AUDIO_DEVICE=cuda:0 TRAIN_DEVICE=cuda:0 bash scripts/e0382_ave_p0_best_to_test_quick_official_panns_lr.sh
#
...
if [[ -z "${BEST_CONFIG_JSON:-}" ]]; then
  ...
  if [[ -n "${latest_best}" && -f "${latest_best}" ]]; then
    ...
    echo "[e0382] BEST_CONFIG_JSON not set; using ${BEST_CONFIG_JSON}"
  else
...
OUT_DIR="${OUT_DIR:-runs/E0382_quick_test402_${EVENTNESS}_$(date +%Y%m%d-%H%M%S)}"
