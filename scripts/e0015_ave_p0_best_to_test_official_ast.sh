...
# Reproduce the best AST config selected by E0014 on official AVE test402.
#
# Usage:
#   BEST_CONFIG_JSON=runs/E0014_.../best_config.json bash scripts/e0015_ave_p0_best_to_test_official_ast.sh
#
# Produces:
#   - runs/E0015_*/metrics.json
...
if [[ -z "${BEST_CONFIG_JSON:-}" ]]; then
  ...
  if [[ -n "${latest_best}" && -f "${latest_best}" ]]; then
    ...
    echo "[e0015] BEST_CONFIG_JSON not set; using ${BEST_CONFIG_JSON}"
  else
...
if [[ -z "${SCORES_JSON:-}" ]]; then
  ...
  if [[ -f "${cand}" ]]; then
    SCORES_JSON="${cand}"
    echo "[e0015] SCORES_JSON not set; using ${SCORES_JSON}"
  fi
fi
...
OUT_DIR="${OUT_DIR:-runs/E0015_ave_p0_best_to_test_official_ast_$(date +%Y%m%d-%H%M%S)}"
